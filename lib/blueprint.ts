...
export const NICHE_BLUEPRINT_TEMPLATES: Record<
  NicheCategory,
  {
    optionA: {
      title: string;
  ...
  "Gaming / Other": {
    ...
    optionB: {
      ...
      features: [
        ...
        "Content calendar with performance tracking and scheduling",
      ],
      whyItFits:
    ...
    optionC: {
      ...
      whyItFits:
        "Most gaming communities leave money on the table. This optimizer tests and refines every monetization lever so you extract maximum value without alienating your core audience.",
    },
  },
};

export const GOAL_MODIFIERS: Record<PrimaryGoal, (features: string[]) => string[]> = {
...
export function calculateChurn(memberCount: number, pricePerMonth: number) {
  const annualLoss = Math.round(memberCount * pricePerMonth * 0.12 * 12);
  const monthlyLoss = Math.round(annualLoss / 12);
  return { annualLoss, monthlyLoss };
}

export function buildBlueprints(state: FunnelState): BlueprintOption[] {
...
export function validateFunnelState(data: unknown): data is FunnelState {
